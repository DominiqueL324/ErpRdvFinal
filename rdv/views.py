...
class RDVApi(APIView):
    ...
    queryset = RendezVous.objects.all().order_by('-date')
    ...
    def get(self,request):
        ...
        if(request.GET.get("user",None) is not None):
            ...
            if request.GET.get("debut",None) is not None and request.GET.get("fin",None) is not None:
                ...
                query_set = RendezVous.objects.filter(client=int(val_)).order_by('-date')
            
            if request.GET.get("en_charge",None) is not None:
                cas = int(request.GET.get("en_charge",None))
                if cas == 0:
                    query_set = RendezVous.objects.filter(Q(statut=1) | Q(statut=float(1)),client=int(val_)).order_by('-date')
                if cas == 1:
                    query_set = RendezVous.objects.filter(Q(statut=2) | Q(statut=float(2)) | Q(statut=3) | Q(statut=float(3)) | Q(statut=4) | Q(statut=float(4)),client=int(val_)).order_by('-date')
        ...
        if(request.GET.get("agent",None) is not None):
            ...
            if request.GET.get("debut",None) is not None and request.GET.get("fin",None) is not None:
                ...
                query_set = RendezVous.objects.filter( Q(agent=int(val_)) | Q(agent_constat=int(val_))| Q(audit_planneur=int(val_)) | Q(agent_constat=float(val_)) ).order_by('-date')
            
            if request.GET.get("en_charge",None) is not None:
                cas = int(request.GET.get("en_charge",None))
                if cas == 0:
                    query_set = RendezVous.objects.filter(Q(statut=1) | Q(statut=float(1)),(Q(agent=int(val_)) | Q(agent_constat=int(val_))| Q(audit_planneur=int(val_)) | Q(agent_constat=float(val_)))).order_by('-date')
                if cas == 1:
                    query_set = RendezVous.objects.filter(Q(statut=2) | Q(statut=float(2)) | Q(statut=3) | Q(statut=float(3)) | Q(statut=4) | Q(statut=float(4)), (Q(agent=int(val_)) | Q(agent_constat=int(val_))| Q(audit_planneur=int(val_)) | Q(agent_constat=float(val_)))).order_by('-date')
        ...
        if(request.GET.get("constat",None) is not None):
            ...
            if request.GET.get("debut",None) is not None and request.GET.get("fin",None) is not None:
                ...
                query_set = RendezVous.objects.filter(Q(agent=int(val_)) | Q(agent_constat=int(val_)) | Q(agent_constat=float(val_)) | Q(audit_planneur=int(val_))).order_by('-date')
            
            if request.GET.get("en_charge",None) is not None:
                cas = int(request.GET.get("en_charge",None))
                if cas == 0:
                    query_set = RendezVous.objects.filter(Q(statut=1) | Q(statut=float(1)),(Q(agent=int(val_)) | Q(agent_constat=int(val_)) | Q(agent_constat=float(val_)) | Q(audit_planneur=int(val_)))).order_by('-date')
                if cas == 1:
                    query_set = RendezVous.objects.filter(Q(statut=2) | Q(statut=float(2)) | Q(statut=3) | Q(statut=float(3)) | Q(statut=4) | Q(statut=float(4)),(Q(agent=int(val_)) | Q(agent_constat=int(val_)) | Q(agent_constat=float(val_)) | Q(audit_planneur=int(val_)))).order_by('-date')
        ...
        if(request.GET.get("planneur",None) is not None):
            ...
            if request.GET.get("debut",None) is not None and request.GET.get("fin",None) is not None:
                ...
                query_set = RendezVous.objects.all().order_by('-date')
            ...
            if request.GET.get("en_charge",None) is not None:
                cas = int(request.GET.get("en_charge",None))
                if cas == 0:
                    query_set = RendezVous.objects.filter(statut=1).order_by('-date')
                if cas == 1:
                    query_set = RendezVous.objects.filter(statut__range=(2,4)).order_by('-date')
        ...
        if(request.GET.get("passeur",None) is not None):
            ...
            if request.GET.get("debut",None) is not None and request.GET.get("fin",None) is not None:
                ...
                query_set = RendezVous.objects.filter(passeur=int(val_)).order_by('-date')
            ...
            if request.GET.get("en_charge",None) is not None:
                cas = int(request.GET.get("en_charge",None))
                if cas == 0:
                    query_set = RendezVous.objects.filter(Q(statut=1) | Q(statut=float(1)),passeur=int(val_)).order_by('-date')
                if cas == 1:
                    query_set = RendezVous.objects.filter(Q(statut=2) | Q(statut=float(2)) | Q(statut=3) | Q(statut=float(3)) | Q(statut=4) | Q(statut=float(4)),passeur=int(val_)).order_by('-date')
        ...
        if request.GET.get("en_charge",None) is not None:
            cas = int(request.GET.get("en_charge",None))
            if cas == 0:
                self.queryset = RendezVous.objects.filter(statut=1).order_by('-date')
            if cas == 1:
                self.queryset = RendezVous.objects.filter(Q(statut=2) | Q(statut=float(2)) | Q(statut=3) | Q(statut=float(3)) | Q(statut=4) | Q(statut=float(4))).order_by('-date')
    ...
    def post(self,request):
        ...
        with transaction.atomic():
            ...
            if request.POST.get("dte_sortie_ancien_loc",None) is not None:
                rdv.dte_sortie_ancien_loc = data['dte_sortie_ancien_loc']
