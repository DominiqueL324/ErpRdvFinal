...
class TrackListingFieldTypeIntervention(serializers.RelatedField):
    def to_representation(self, value):
        result = {}
        if value == None:
            inter = TypeIntervention.objects.filter(pk=7).first()
            result['type'] = inter.type
            result['statut']= inter.statut,
            result['created_at'] =inter.created_at,
            result['id']=inter.id
        else:
            result['type'] = value.type
            result['statut']= value.statut,
...
class ProprieteRepresentation(serializers.RelatedField):
    def to_representation(self, value):
        ...
        if value.type_propriete == None:
            pro = TypePropriete.objects.filter(pk=7).first()
            result['type_propriete'] = {
                "type": pro.type,
                "statut":pro.statut,
                "id":pro.id
                },
            ...
            result['type_propriete'] = {
                "type": value.type_propriete.type,
                "statut":value.type_propriete.type,
                "id":value.type_propriete.id
            }
        return result
